...
def GenCh1(f, val):
    if val == 'W':
        img = Image.new("RGB", (27, 70), (255, 255, 255))
        draw = ImageDraw.Draw(img)
...
class GenPlate:
    def __init__(self, fontCh, fontEng, NoPlates, background):
        self.fontC = ImageFont.truetype(fontCh, 43, 0)
        self.fontE = ImageFont.truetype(fontEng, 60, 0)
        self.img = np.array(Image.new("RGB", (226, 70), (255, 255, 255)))
        self.bg = cv2.resize(cv2.imread(background), (226, 70))
        self.noplates = []
        for parent, parent_folder, filenames in os.walk(NoPlates):
            for filename in filenames:
                path = parent + "/" + filename
                img = cv2.imread(path)
                img = cv2.resize(img, (286, 70))
                self.noplates.append(img)
    ...
    def generate(self, text):
        if len(text) == 9:
            ...
            com = tfactor(com)
            com = random_envirment(com, self.noplates)
            com = AddGauss(com, 1 + r(3))
...
if __name__ == '__main__':
    gen = GenPlate("./font/platech.ttf", './font/platechar.ttf', "./NoPlates", "./images/template.bmp")
    gen.genBatch(10, "./plate", (185, 50))
